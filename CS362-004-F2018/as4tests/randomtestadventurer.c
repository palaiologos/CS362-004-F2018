...
int main() {
  ...
  for (i = 0; i <= 1000; i++) {
    ...
    G.numBuys = rand() % 5;
    G.coins = rand() % 5;
    // Set player's first card to be adventurer.
    ...
    // Then, check actions, discard and those stats.
    // if num actions is the same.
    if (G.numActions == testGameState.numActions) {
    ...
    // Discard count is not equal.
    if (G.discardCount[currentPlayer] != testGameState.discardCount[currentPlayer]) {
    ...
    printf("Random test %d done. %d tests passed out of 2\n", i, tests_passed);
